...
/**
 * Project02Particles Component
 * 
 * Dedicated 3D spatial particle environment for Project 02: Visual Communication.
 * 
 * Key Architecture:
 *  1. Prominent Symmetrical Circular Waves:
 *     - Entry Wave (0.0s to 1.35s): High-displacement circular wave sweeps across particles.
 *     - Exit Wave (0.0s to 0.70s): Circular wave sweeps outward across the field.
 *  2. Pure Forward 3D Travel:
 ...
 *     - Scroll tracking smoothly drives camera through the 3D particle space.
 */
export default function Project02Particles({
  scrollProgress = 0,
  ...
  useEffect(() => {
    ...
    const loop = (timestamp) => {
      ...
      // 3D Travel Depth tied directly to Project 02 scroll
      ...
      // -------------------------------------------------------------
      // Phase 2: Prominent Spherical Wave Exit (0.0s to 0.70s)
      // -------------------------------------------------------------
      let exitProgress = 0.0;
      if (currentLifecycle === 'CLOSING' && state.exitStartTime) {
        const exitElapsed = (timestamp - state.exitStartTime) / 1000;
        exitProgress = Math.min(1.0, exitElapsed / 0.70);
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        if (relZ > 50 && relZ < 3000) {
          ...
          // -----------------------------------------------------------
          // Prominent Spherical Wave Arrival Physics (Home -> Project 02)
          // -----------------------------------------------------------
          if (currentLifecycle === 'OPENING' && openProgress < 1.0) {
            ...
            if (waveDistDiff < bandWidth) {
              ...
              // Outward displacement along radial vector
          ...
          // -----------------------------------------------------------
          // Prominent Spherical Wave Departure Physics (Project 02 -> Home)
          // -----------------------------------------------------------
